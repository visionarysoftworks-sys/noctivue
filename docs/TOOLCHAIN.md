...
- **Manifest format:** Decided (ADR-017) — a file named
  **`nestpkg.nvpm`** in a custom Noctivue-flavored declarative syntax
  (colon blocks + significant indentation), read by a dedicated parser
  in `noct-cli` (not the full compiler frontend). Closes the former
  Open question (TOML-like vs. `.nv`-native).
- **Lockfile:** one canonical, checked-in lockfile format, generated by
  `noct add`/`noct build`, ensuring reproducible builds. Named
  **`nestpkg.lock`** (ADR-017); format decided alongside the manifest
  parser.
