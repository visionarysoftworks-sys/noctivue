...
impl ModuleGraph {
    ...
    fn root_import_aliases(&self) -> HashMap<String, String> {
        ...
        for import in &root.program.imports {
            if let Some((_, item)) = self.resolve_import(root, import) {
                if item.is_none() {
                    // Unaliased module imports are flattened into the linked
                    // program. Only an explicit alias introduces a qualified
                    // module name, so it cannot collide with local variables
                    // such as `document` or `workspace`.
    ...
    fn resolve_import(
        ...
        if let Ok(exact) = resolve_path(&importer.path, &path) {
...
fn resolve_path(importer: &Path, import: &str) -> std::io::Result<PathBuf> {
    ...
    while let Some(dir) = ancestor {
        ...
        // checked-out sibling package before falling back to the package
        // cache, while retaining the current package's `lib` convention.
        if parts.len() > 1 {
            let package = parts[0];
            ...
            let cache = dir.join(".noct").join("packages");
            if let Ok(entries) = std::fs::read_dir(cache) {
                for entry in entries.flatten() {
                    let name = entry.file_name();
                    let name = name.to_string_lossy();
                    if name == package || name.starts_with(&format!("{package}-")) {
                        candidates.push(entry.path().join("lib").join(&module));
                    }
                }
            }
        }
...
fn rewrite_item(item: &mut Item, aliases: &HashMap<String, String>) {
    match item {
        Item::Function(f) => {
            if let FunctionBody::Block(b) = &mut f.body {
                rewrite_block(b, aliases);
            }
            if let FunctionBody::Expr(e) = &mut f.body {
                rewrite_expr(e, aliases);
            }
        }
        Item::BareDecl(d) => rewrite_block(&mut d.body, aliases),
        Item::Task(t) => rewrite_block(&mut t.body, aliases),
